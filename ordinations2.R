...
#Arrange data
#env_scaled <- filter(env_means_scaled, ID %in% rownames(VF) )

common <- intersect(env_means_scaled$ID, VF$ID)
veg.ord <- filter(VF, ID %in% common)
env.ord <- filter(env_means_scaled, ID %in% common)
env.ord$n <- NULL
env.ord$p <- NULL
env.ord$N.P <- NULL
env.ord$sum_canopy <- NULL
combo <- left_join(veg.ord, env.ord, by ="ID")
combo$survey_year.y <- NULL
combo <- rename(combo, survey_year = survey_year.x)
#extra <- dplyr::select(dat.all.countries, c(ID, mean_age, tree))
...
combo <- ungroup(combo)
combo2 <- drop_na(combo)

env.ord <- dplyr::select(combo2, ID, latitude, longitude, n_nh4, n_no3,
                         mean_temp, mean_precip,survey_year) 
spp.ord <- dplyr::select(combo2, -c(latitude, longitude, n_nh4, n_no3,survey_year,
                                    mean_temp, mean_precip, ID_site,
                                    survey_year, country, code_country))
