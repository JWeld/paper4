...
Y <- as.data.frame(sign(ordispe))
Y <- as.matrix(sign(ordispe))

summary(rowSums(Y))
summary(colSums(Y))

X <- as.data.frame(ordienv)
X <- ordienv_scaled
...
fit_base <- gllvm(Y, num.lv = 2, family = "binomial")


fit <- Hmsc(Y, XData = X, XFormula = ~NH4M + NO3M + SO4SM, distr="probit")
