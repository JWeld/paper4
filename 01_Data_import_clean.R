...
#Import VG FD (Intensive plots and Forest damage, we only use the former) #1990 - 2018 ####
...
# average per site/year combo. First sum annual total for each station and then take 
# mean of station values to get a whole site value (some sites have multiple stations reporting).
...
#COMBINE data - species and environemntal matrixes####
...
ordi_extras <-  select(ungroup(test), ID_fine2, ID_plot, ID_site, NH4M, NO3M, SO4SM, latitude, longitude, 
                   PREC, survey_year) %>% column_to_rownames(var="ID_fine2")

ordi_extras.df <-  select(ungroup(test), ID_fine2, ID_plot, ID_site, NH4M, NO3M, SO4SM, latitude, longitude, 
                       PREC, survey_year)
extras <- test %>% 
...
ordispe.df <- select(test, -c(survey_year,ID,ID_site,ID_plot,ID_subplot,ID_fine, 
                              NH4M,NO3M,SO4SM,PREC,latitude,longitude)) %>%
