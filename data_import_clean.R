...
VG_ground <- VG_ground %>% group_by(survey_year,ID,ID_site,ID_fine,ID_fine2, Medium) %>%
  summarise(Cover = sum(Value)) %>% rename(Species = Medium) %>% ungroup()
...
VG_f_wide <- VG_f_wide[rowSums(VG_f_wide[,-c(1:9)]) != 0, ]
VG_b_wide <- VG_b_wide[rowSums(VG_b_wide[,-c(1:9)]) != 0, ]
VG_g_wide <- VG_g_wide[rowSums(VG_g_wide[,-c(1:9)]) != 0, ]
...
VG_fm <- select(VG_f_wide, -c(1:5))
VG_fm <- column_to_rownames(VG_fm, var = "ID_fine2")

VG_bm <- select(VG_b_wide, -c(1:5))
VG_bm <- column_to_rownames(VG_bm, var = "ID_fine2")

VG_gm <- select(VG_g_wide, -c(1:4))
VG_gm <- column_to_rownames(VG_gm, var = "ID_fine2")
